...
public class MyDataRestConfig implements RepositoryRestConfigurer {
    ...
    public void configureRepositoryRestConfiguration(RepositoryRestConfiguration config, CorsRegistry corsRegistry) {
        ...
        disbaleHttpMethods(Review.class, config, theUnsupportedActions);  // 지원하지 않는 메소드 비활성화

        corsRegistry.addMapping(config.getBasePath() + "/**").allowedOrigins(theAllowedOrigins);    // CORS 설정
    }

    private void disbaleHttpMethods(Class<?> theClass, RepositoryRestConfiguration config, HttpMethod[] theUnsupportedActions) {
